...
- [ ] `POST /boards/:boardId/lists` - 创建列表
  - [ ] **权限要求**: 看板 ADMIN/MEMBER 或工作区 OWNER/MEMBER
  - [ ] **权限检查**: 使用 `@RequireBoardRole(['ADMIN', 'MEMBER'])`
  - [ ] **权限继承**: 自动继承看板权限
  - [ ] **排序处理**: 自动计算新列表的排序位置

- [ ] `GET /boards/:boardId/lists` - 获取列表
  - [ ] **权限要求**: 看板访问权限 (任何角色)
  - [ ] **权限检查**: 使用 BoardGuard 验证看板访问权限
  - [ ] **数据返回**: 按排序返回列表，包含卡片数量

- [ ] `PATCH /lists/:id` - 更新列表
  - [ ] **权限要求**: 看板 ADMIN/MEMBER 权限
  - [ ] **权限检查**: 通过列表ID反查看板权限
  - [ ] **操作限制**: VIEWER 角色无法修改
...
- [ ] `PATCH /cards/:id` - 更新卡片
  - [ ] **权限要求**: 看板 ADMIN/MEMBER 权限 或 卡片分配者
  - [ ] **权限检查**: 看板权限 + 个人分配权限
  - [ ] **字段限制**: 不同角色可编辑不同字段
  - [ ] **分配验证**: 只能分配给看板成员

- [ ] `DELETE /cards/:id` - 删除卡片
  - [ ] **权限要求**: 看板 ADMIN 权限 或 卡片创建者
  - [ ] **权限检查**: 管理员权限 + 创建者权限
  - [ ] **排序更新**: 自动更新其他卡片排序

- [ ] `PATCH /cards/:id/move` - 移动卡片
  - [ ] **权限要求**: 看板 ADMIN/MEMBER 权限
  - [ ] **权限检查**: 验证源列表和目标列表都在同一看板
  - [ ] **跨看板移动**: 需要两个看板的权限验证
  - [ ] **排序重算**: 更新两个列表的卡片排序
